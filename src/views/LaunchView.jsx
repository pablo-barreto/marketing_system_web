...
const LaunchView = ({ crmData }) => {
  ...
  // -----------------------------------------------------------------------
  // 2. CAMPAÑAS ACTIVAS (para modo "Enviar a existentes")
  // -----------------------------------------------------------------------
  ...
  const handleGlobalLaunch = async (file, title, mode, roles, budget, targetCountries = [], targetServices = []) => {
    ...
    try {
      ...
      formData.append('segmentation_mode', mode);
      ...
      Swal.fire({ title: '¡Lanzamiento Exitoso!', text: successMsg, icon: 'success' });
    } catch (error) {
      console.error(error);
  ...
  // B. Sincronizar audiencia a campañas existentes
  const handleSyncAudience = async (mode, roles, campaignIds) => {
    setLoading(true);
    try {
      const body = {
        campaign_ids: campaignIds,
        segmentation_mode: mode,
        target_roles: roles ? roles.split(',').filter(Boolean) : []
      };

      await launchService.syncAudienceToCampaigns(body, basicAuthHeader);

      Swal.fire({
        title: '¡Sincronización Encolada!',
        text: `La audiencia se está sincronizando a ${campaignIds.length} campaña(s). Recibirás una notificación al finalizar.`,
        icon: 'success',
  ...
  // C. Sincronización de Lookalikes
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm h-fit">
          ...
          <ul className="space-y-4 text-sm text-slate-600">
            ...
            <li className="flex gap-3">
              ...
              <div>
                <strong>Enviar a Existentes:</strong> Sincroniza la audiencia segmentada a campañas ya creadas sin generar nuevas.
              </div>
            </li>
          </ul>
        </div>
